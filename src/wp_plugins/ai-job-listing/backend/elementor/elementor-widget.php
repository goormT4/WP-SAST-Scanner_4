...
class Axilweb_Ajl_jobs_Widget {

    /**
     * Constructor to initialize hooks and actions.
     */
    public function __construct() {
        add_action('elementor/widgets/register', [$this, 'axilweb_ajl_jobs_register_widget']);
        add_shortcode('ai-job-listing', [$this, 'axilweb_ajl_jobs_shortcode_callback']);
    }
     ...
     * @return void
     */
    public function axilweb_ajl_jobs_register_widget($widgets_manager) {
     ...
     */
    private function axilweb_ai_job_listing_get_jobs_from_api($settings) {
      $base_url = home_url() . '/wp-json/ai_job_listing/v1/jobs-frontend';
        ...
        if ($response_code !== 200) {
            ...
            error_log('AI Job Listing Elementor Legacy API Error: Code ' . $response_code . ', Body: ' . $response_body . ', URL: ' . $url);
     ...
     */
    private function axilweb_ai_job_listing_render_jobs_html($results) {
     ...
     */
    public function axilweb_ajl_jobs_shortcode_callback($atts) {
        $defaults = array(
            'job_limit' => 2,
            'ordering' => 'ASC',
        );
        $atts = shortcode_atts($defaults, $atts, 'ajl-job-listing');
        $response = $this->axilweb_ai_job_listing_get_jobs_from_api($atts);
        ...
        return $this->axilweb_ai_job_listing_render_jobs_html($results);
    } 

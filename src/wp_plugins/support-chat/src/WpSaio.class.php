...
class WpSaio
{
    ...
    public static function defaultAppsWithCustomApps()
    {
        $results = [];
        $default_apps = self::defaultApps(false);
        $added_apps = get_option( 'njt_wp_saio', array() );
        if( ! is_array($added_apps) ) {
            $added_apps = array();
        }

        //append custom-app-xxx to $default_apps
        foreach($added_apps as $appKey => $appData) {
            if(strpos($appKey, 'custom-app-') !== false) {
                $default_apps[$appKey] = [
                    ...
                    'title' => esc_html__('Custom App', 'support-chat'),
                    'shortcode' => '',
                    'params' => [
                        'url' => [
                            'placeholder' => esc_html__('Enter your URL', 'support-chat'),
                            'desc' => esc_html__('Example: https://yourapp.com/', 'support-chat'),
                            'custom-app-title' => $appData['params']['custom-app-title'],
                            'url-icon' => $appData['params']['url-icon'],
                            'color-icon' => $appData['params']['color-icon'],
                        ]
                    ]
                ];
            }
        ...
        foreach($default_apps as $appKey => $appData) {
            $paramKeys = array_keys($appData['params']);
            $paramKey = $paramKeys[0]; //url or phone or username
            $paramValue = $appData['params'][$paramKey];

            $app = [
                'icon' => $appData['icon'],
                'title' => $appData['title'],
                'placeholder' => $paramValue['placeholder'],
                'desc' => $paramValue['desc'],
                'state' => 'app_off',
                'key' => $paramKey
            ];

            if(isset($added_apps[$appKey])) {
                $app['value'] = $added_apps[$appKey]['params'][$paramKey] ?? '';
                $app['state'] = $added_apps[$appKey]['params']['state'] ?? 'app_off';
                if( strpos( $appKey, 'custom-app' ) !== false) {
                    $app['custom_app_title'] = $added_apps[$appKey]['params']['custom-app-title'] ?? __('Custom App', 'support-chat');
                    $app['url_icon'] = $added_apps[$appKey]['params']['url-icon'] ?? '';
                    $app['color_icon'] = $added_apps[$appKey]['params']['color-icon'] ?? '';
                }
            }
    ...
    public static function addedAppsOrder() {
        $added_apps = get_option( 'njt_wp_saio', array() );
        if( ! is_array( $added_apps ) ) {
            $added_apps = [];
        }
        $keys = array_keys( $added_apps );
        $default_apps = self::defaultApps(false);
        foreach($default_apps as $appKey => $app) {
            if( ! isset( $added_apps[$appKey] ) ) {
                $keys[] = $appKey;
            }
        }
    ...
    public static function defaultApps($from_db = true)
    {
        $default_apps =  array(
            'facebook-messenger' => array(
                ...
                'title' => esc_html__('Messenger', 'support-chat'),
                'shortcode' => 'wpsaio_facebook_messenger',
                'params' => array(
                    'url' => array(
                        'type' => 'text',
                        'required' => true,
                        'placeholder' => esc_html__('Username', 'support-chat'),
                        'desc' => esc_html__('Example: https://m.me/username or username', 'support-chat'),
                    ),
                ),
            ),
            'whatsapp' => array(
                ...
                'title' => esc_html__('WhatsApp', 'support-chat'),
                'shortcode' => 'wpsaio_whatsapp',
                'params' => array(
                    'phone' => array(
                        'type' => 'text',
                        'required' => true,
                        'placeholder' => esc_html__('Phone number', 'support-chat'),
                        'desc' => esc_html__('Example: +17141234567', 'support-chat'),
                    ),
                ),
            ),
            'snapchat' => array(
                ...
                'title' => esc_html__('Snapchat', 'support-chat'),
                ...
                'params' => array(
                    'username' => array(
                        ...
                        'required' => true,
                        'placeholder' => esc_html__('Username', 'support-chat'),
                        'desc' => esc_html__('Example: username', 'support-chat'),
                    ),
                ),
            ),
            'line' => array(
                ...
                'title' => esc_html__('Line', 'support-chat'),
                'shortcode' => 'wpsaio_line',
                'params' => array(
                    'url' => array(
                        'type' => 'text',
                        'required' => true,
                        'placeholder' => esc_html__('Line ID', 'support-chat'),
                        'desc' => esc_html__('Example: https://line.me/ti/p/your-line-id', 'support-chat'),
                    ),
                ),
            ),
            'viber' => array(
                ...
                'title' => esc_html__('Viber', 'support-chat'),
                'shortcode' => 'wpsaio_viber',
                'params' => array(
                    'account' => array(
                        'type' => 'text',
                        'required' => true,
                        'placeholder' => esc_html__('Phone number, Community URL, or Bot URL', 'support-chat'),
                        'desc' => esc_html__('Example: +17141234567, https://vb.me/vibercommunity, or viber://pa?chatURI=myshopbot', 'support-chat'),
                    ),
                ),
            ),
            'phone' => array(
                ...
                'title' => esc_html__('Phone', 'support-chat'),
            ...
            'email' => array(
                ...
                'title' => esc_html__('Email', 'support-chat'),
                'shortcode' => 'wpsaio_mail',
                'params' => array(
                    'email' => array(
                        'type' => 'text',
                        'required' => true,
                        'placeholder' => esc_html__('Email address', 'support-chat'),
                        'desc' => esc_html__('Example: support@ninjateam.org', 'support-chat'),
                    ),
                ),
            ),
            'telegram' => array(
                ...
                'title' => esc_html__('Telegram', 'support-chat'),
                'shortcode' => 'wpsaio_telegram',
                'params' => array(
                    'username' => array(
                        'type' => 'text',
                        'required' => true,
                        'placeholder' => esc_html__('Telegram account', 'support-chat'),
                        'desc' => esc_html__('Example: username', 'support-chat'),
                    ),
                ),
            ),
			...
			c1.37,0,2.49,1.12,2.49,2.49C23.53,14.1,22.41,15.22,21.04,15.22z"/></svg>',
                'title' => esc_html__('Zalo', 'support-chat'),
                'shortcode'=> 'wpsaio_zalo',
                'params' => array(
                    'username' => array(
                        'type' => 'text',
                        'required' => true,
                        'placeholder' => esc_html__('Phone number or Zalo Official Account ID', 'support-chat'),
                        'desc' => esc_html__('Example: 0987123456 or 18168081905191', 'support-chat'),
                    ),
                ),
            ),
            'kakaotalk' => array (
		...
		l-1.71,1.71l1.85,2.45C22.5,13.76,22.56,13.96,22.53,14.16z"/></svg>',
                'title' => esc_html__('Kakaotalk', 'support-chat'),
                'shortcode'=> 'wpsaio_kakaotalk',
                'params' => array(
                    'username' => array(
                        'type' => 'text',
                        'required' => true,
                        'placeholder' => esc_html__('Kakaotalk account', 'support-chat'),
                        'desc' => esc_html__('Example: channel_id', 'support-chat'),
                    ),
                ),
            ),
            'wechat' => array (
		...
		c-0.12,0.44-0.46,0.79-0.9,0.9V9.08z"/></svg>',
                'title' => esc_html__('Wechat', 'support-chat'),
                'shortcode'=> 'wpsaio_wechat',
                'params' => array(
                    'email' => array(
                        'type' => 'text',
                        'required' => true,
                        'placeholder' => esc_html__('Wechat account', 'support-chat'),
                        'desc' => esc_html__('Example: user_id', 'support-chat'),
                    ),
                ),
            ),
            'custom-app' => array(
                ...
                'params' => array(
                    'url' => array(
                        ...
                        'placeholder' => esc_html__('Enter your URL', 'support-chat'),
                        'desc' => esc_html__('Example: https://yourapp.com/', 'support-chat'),
                        'desc-upload-icon' => esc_html__('Your app icon/image', 'support-chat'),
                        'custom-app-title' => '',
                        'url-icon' => '',
                        'color-icon' => '#007cc4'
                    ),
        ...
        // update_option('njt_wp_saio_default_apps', $apps);
        $apps = get_option('njt_wp_saio_default_apps', $default_apps);
        
        // Merge $apps with $default_apps: keep all attributes from $apps, but placeholder and desc from $default_apps
        foreach ($apps as $key => $app) {
            if (isset($default_apps[$key]) && isset($app['params'])) {
                foreach ($app['params'] as $param_key => $param) {
                    if (isset($default_apps[$key]['params'][$param_key]['placeholder'])) {
                        $apps[$key]['params'][$param_key]['placeholder'] = $default_apps[$key]['params'][$param_key]['placeholder'];
                    }
                    if (isset($default_apps[$key]['params'][$param_key]['desc'])) {
                        $apps[$key]['params'][$param_key]['desc'] = $default_apps[$key]['params'][$param_key]['desc'];
                    }
                }
            }
        }
        
        //remove app with key skype
        unset($apps['skype']);
        return $apps;
    }
    ...
    public static function renderShortcodes()
    {
        ...
        foreach ($apps as $k => $v) {
            if (in_array($k, array_keys($from_db))) {
                ...
                if( str_contains($k, 'custom-app') !== false ) {
                    $atts[] = 'custom-app-key="' . esc_attr($k) . '"';
                }
                $atts = implode(' ', $atts);
    ...
    public static function renderInput($app, $key, $input)
    {
        ...
        if (str_contains($app, 'custom-app')) {
            ...
            <span class="button wp_saio_choose_image_btn saio-custom-app-upload-icon-btn" data-target="#'.$id .'">'. esc_html__('Upload Icon', 'support-chat') .'</span>
...
?>
        <div class="wp_saio_panel_wrap">
            <p><?php echo esc_html__('Select app icons to add them to your list.', 'support-chat') ?></p>
            <?php
            foreach ($apps as $k => $v) {
        ...
        <div style="margin-bottom: 25px">
            <a href="javascript:void(0)" class="button" id="add-new-custom-app"><?php echo esc_html__('Add New App', 'support-chat') ?></a>
        </div>
        ...
        <div class="wp_saio_panel_btn-wrap">
            <button class="wpsaio-save button button-primary button-choose-apps"><?php echo esc_html__('Save Changes', 'support-chat') ?><i class="dashicons dashicons-update-alt"></i></button>
        </div>
<?php
    }
